...
/* [PNEUMA] 2026-03-31 - Temporal liberation layer for LLM-composed music.
 * %%PNEUMA directives are stored as MIDI text meta-events with a "PNEUMA:"
 * prefix so that an external post-processor (pneuma) can read them from the
 * MIDI output and apply biological/organic timing transformations.
 * Recognized directives:
 *   %%PNEUMA heartbeat BPM variance
 *   %%PNEUMA drift rate
 *   %%PNEUMA rubato beat_weights...
 *   %%PNEUMA breathe bars
 *   %%PNEUMA polytempo V:N ratio
 *   %%PNEUMA humanize ticks
 *   %%PNEUMA free start|end
 *   %%PNEUMA gravity phrase_weight
 *   %%PNEUMA couple velocity_timing_factor
 */
static void event_pneuma(char *s)
{
  char msg[256];
  char command[40];
  char *p;
  ...
  /* validate that the directive starts with a known keyword */
  readstr(command, &p, 40);
  if (strcmp(command, "heartbeat") != 0 &&
      strcmp(command, "drift") != 0 &&
      strcmp(command, "rubato") != 0 &&
      strcmp(command, "breathe") != 0 &&
      strcmp(command, "polytempo") != 0 &&
      strcmp(command, "humanize") != 0 &&
      strcmp(command, "free") != 0 &&
      strcmp(command, "gravity") != 0 &&
      strcmp(command, "couple") != 0) {
    char errmsg[100];
    snprintf(errmsg, sizeof(errmsg),
             "%%%%PNEUMA command \"%s\" not recognized", command);
...
/* package-specific command found i.e. %%NAME */
/* %%MIDI and %%PNEUMA commands are handled */
char *package, *s;
{
  char  command[40];
  char *p;
  ...
  /* [PNEUMA] 2026-03-31 - PNEUMA directives in file header are stored
   * for later emission at the start of MIDI track 0 */
  if (strcmp(package, "PNEUMA") == 0) {
